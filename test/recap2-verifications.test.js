...
const studio = (nom) => ({
  ...
  clientsRetrouves: {
    ...
    liste: [
      ...
      ligne({ client: 'Fiona Del Giudice', date: '12/08/2026', site: 'My Coach Marcq', idClient: '42140' }),
      // La vente « à vérifier » : fiche Deciplus trouvée, aucune vente saisie.
      ligne({ client: 'Esther Jhureea', date: '24/08/2026', retrouve: false, site: '', dateVente: '', idClient: '', ficheId: '42350' }),
    ],
  },
...
const verifier = (o, auth = true) => fetch(BASE + '/api/recap2/verification', {
  ...
  body: JSON.stringify(Object.assign({ mois: '2026-08', studio: 'Wasquehal', client: 'Esther Jhureea', date: '24/08/2026', valeur: 'verifiee' }, o)),
});
const esther = (r, s = 'Wasquehal') => r.studios[s].clientsRetrouves.liste.find((l) => l.client === 'Esther Jhureea');
...
test('au départ : la vente est « à vérifier », 1/2 contrat validé', async () => {
  const r = await lire();
  assert.equal(esther(r).retrouve, false);
  assert.equal(esther(r).verification, undefined);
...
test('le pourcentage passe à 2/2, sans que « retrouvée » ne bouge', async () => {
  const r = await lire();
  assert.equal(esther(r).verification.verifiee, true);
  assert.equal(esther(r).retrouve, false, 'la recherche automatique reste intacte — traçabilité');
  assert.equal(M.motifValidation(esther(r)), 'verification', 'jamais « automatique » ni « rapprochement »');
  const cv = M.contratsValides(r.studios.Wasquehal.clientsRetrouves.liste);
  assert.deepEqual([cv.valides, cv.actives, cv.taux], [3, 3, 1], '3/3 = 100 %');
  assert.equal(esther(r).ficheId, '42350', 'le lien vers la fiche Deciplus reste possible');
});
...
test('aucun KPI du rapport ne bouge, et le fichier sur disque reste brut', async () => {
  ...
  const e = brut.studios.Wasquehal.clientsRetrouves.liste.find((l) => l.client === 'Esther Jhureea');
...
test('un autre studio, même nom : sa vente reste à vérifier', async () => {
  const r = await lire();
  assert.equal(esther(r, 'Lille').verification, undefined);
});

test('une vente DÉJÀ retrouvée ne peut pas être vérifiée à la main', async () => {
  const rep = await verifier({ client: 'Mustapha Teir', date: '12/08/2026' });
...
test('NOUVEAU CONTRÔLE AUTOMATIQUE déposé : la vérification tient', async () => {
  const depot = { controleLe: '2026-09-17T18:30', resultats: [
    { studio: 'Wasquehal', client: 'Esther Jhureea', date: '24/08/2026', idDeciplus: '42350',
  ...
  const r = await lire();
  assert.equal(esther(r).verification.verifiee, true);
  assert.equal(esther(r).operationnel.prelevement, 'ok');
  assert.equal(kpi(r), KPI0);
});
...
test('ANNULER : la vente redevient « à vérifier » et sort du pourcentage', async () => {
  ...
  const r = await lire();
  assert.equal(esther(r).verification, undefined);
  assert.equal(M.motifValidation(esther(r)), '');
...
//  Fiona Del Giudice est retrouvée à Wasquehal, mais sur le site Deciplus de
//  Marcq. L'écart est une ALERTE : un administrateur peut la marquer vérifiée.
const fiona = (r, s = 'Wasquehal') => r.studios[s].clientsRetrouves.liste.find((l) => l.client === 'Fiona Del Giudice');
const acquitter = (o, auth = true) => fetch(BASE + '/api/recap2/verification', {
  method: 'POST', headers: Object.assign({ 'Content-Type': 'application/json' }, auth ? { Authorization: 'Bearer ' + jeton } : {}),
  body: JSON.stringify(Object.assign({ mois: '2026-08', studio: 'Wasquehal', client: 'Fiona Del Giudice', date: '12/08/2026',
    valeur: 'verifiee', type: 'site' }, o)),
});

test('au départ : l\'écart de site est une alerte à traiter', async () => {
  const r = await lire();
  assert.deepEqual(M.siteDivergent(fiona(r), 'Wasquehal'), { attendu: 'Wasquehal', site: 'My Coach Marcq' });
  assert.equal(fiona(r).ecartSite, undefined);
});
...
test('MARQUER COMME VÉRIFIÉ : tracé, sans toucher au site ni à l\'attribution', async () => {
  ...
  const r = await lire();
  assert.equal(fiona(r).ecartSite.verifiee, true);
  assert.equal(fiona(r).site, 'My Coach Marcq', 'le site Deciplus trouvé reste inchangé — traçabilité');
  assert.equal(fiona(r).studio, undefined, 'aucune réattribution de la vente');
  assert.deepEqual(M.siteDivergent(fiona(r), 'Wasquehal'), { attendu: 'Wasquehal', site: 'My Coach Marcq' },
...
test('l\'acquittement d\'un écart ne vaut pas validation de vente', async () => {
  const r = await lire();
  assert.equal(fiona(r).verification, undefined);
  assert.equal(M.motifValidation(fiona(r)), 'automatique', 'elle était déjà retrouvée');
});

test('une vente SANS écart de site ne peut pas être acquittée', async () => {
  assert.equal((await acquitter({ client: 'Mustapha Teir', date: '12/08/2026' })).status, 409);
});

test('une vente NON retrouvée ne peut pas porter un écart de site', async () => {
  assert.equal((await acquitter({ client: 'Esther Jhureea', date: '24/08/2026' })).status, 409);
});
...
test('l\'acquittement tient après une nouvelle collecte et un redémarrage', async () => {
  assert.equal((await deposer(rapport())).status, 200);
  assert.equal(fiona(await lire()).ecartSite.verifiee, true);
