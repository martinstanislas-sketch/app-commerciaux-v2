'use strict';
// ============================================================================
//  MY COACH ACADEMY — amorçage de « Prévenir le décrochage et réengager un client ».
//
...
//  « Prévention du décrochage » n'ouvrira aucun dossier client du Boost.
//
...
const BANQUE = require('./academyBanquePrevenirDecrochage');
const { idYoutubeValide } = require('./academy');

const USAGE_MINI = 'mini';
const USAGE_FINALE = 'finale';

function createAmorcagePrevenirDecrochage({ getDb, nowIso, formations, qcm, pratique }) {
  const db = () => getDb();
  ...
  function amorcerCas() {
    ...
    try {
      ...
      console.warn('⚠️  Prévenir le décrochage : cas pratiques non amorcés — ' + e.message + '.');
      return 0;
    }
    return poses;
  }
  ...
  function amorcerConsignesCas() {
    ...
    try {
      ...
      console.warn('⚠️  Prévenir le décrochage : consignes des cas non posées — ' + e.message + '.');
      return 0;
    }
    return remplies;
  }
  ...
  function amorcer() {
    ...
    if (cas) console.log(`  Academy : « Prévenir le décrochage » — ${cas} cas pratiques posés.`);
    const cons = amorcerConsignesCas();
    if (cons) console.log(`  Academy : « Prévenir le décrochage » — ${cons} consignes de cas renseignées.`);
    ...
    for (const m of BANQUE.MODULES) {
      if (!idYoutubeValide(m.youtubeId)) {
        console.warn(`⚠️  Prévenir le décrochage : identifiant YouTube invalide au module ${m.ordre} — amorçage annulé.`);
        return 0;
      }
      if (vus.has(m.youtubeId)) {
        console.warn(`⚠️  Prévenir le décrochage : identifiant YouTube en double au module ${m.ordre} — amorçage annulé.`);
        return 0;
      }
      vus.add(m.youtubeId);
    }
    ...
    try {
      d.transaction(() => {
        ...
        BANQUE.FINALE.forEach((q, i) => {
          ajouts += poser(`pd-fin-q${String(i + 1).padStart(2, '0')}`, null, USAGE_FINALE, i + 1, q);
        });
      ...
      if (e && e.message === 'ANNULER') {
        console.warn('⚠️  Prévenir le décrochage : amorçage annulé — ' + (echec || 'formation refusée') + '.');
        return 0;
      }
      console.warn('⚠️  Prévenir le décrochage : amorçage impossible — ' + e.message + '. L\'app démarre normalement.');
      return 0;
    }
    ...
    if (casApres) console.log(`  Academy : « Prévenir le décrochage » — ${casApres} cas pratiques posés.`);
    const consApres = amorcerConsignesCas();
    if (consApres) console.log(`  Academy : « Prévenir le décrochage » — ${consApres} consignes de cas renseignées.`);
    return ajouts;
  }
...
module.exports = { createAmorcagePrevenirDecrochage, BANQUE };
