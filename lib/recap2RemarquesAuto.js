...
function enregistrer(db, { mois, studio, type, ligne, texteAuto, texte, par = '' }, maintenant = new Date().toISOString()) {
  ...
  if (TYPES.indexOf(type) < 0) throw new Error('type : vente ou vni');
...
function appliquer(rapport, index) {
  ...
  Object.keys(copie.studios).forEach((s) => {
    ...
    [['vente', b.clientsRetrouves], ['vni', b.vni]].forEach(([type, bloc]) => {
      ((bloc && bloc.liste) || []).forEach((l) => {
        const rows = index.get(s + '|' + type + '|' + clePersonne(type, l));
        if (!rows || !rows.length) return;
        l.remarquesAuto = {};
        rows.forEach((r) => { l.remarquesAuto[r.texte_auto] = { texte: r.texte, modifieLe: r.modifie_le, modifiePar: r.modifie_par }; });
      });
    });
  });
  return copie;
