...
const CLES_CR_LIGNE = ['client', 'date', 'prestation', 'commercial', 'annulee', 'dateAnnulation',
  ...
  'paiement', 'ficheId', 'ficheNom', 'ficheSite', 'contactIdVendor', 'idDeciplusVendor'];
// `contactIdVendor` / `idDeciplusVendor` (FACULTATIFS, ajoutés le 2026-09-16) :
// le contact Vendor de la vente et l'Id Deciplus lu sur SA fiche Vendor —
// chaînage déterministe, sans nom. Admis sur TOUTE ligne (retrouvée, « à
// vérifier », annulée) : ils n'ont aucun effet sur `retrouve`, `idClient` ni
// sur un KPI. Absents dans les rapports d'avant.
  ...
  if (cr !== null && cr !== undefined) {
    ...
    if (!estObjet(cr)) pb.push(ou + ' : objet ou null attendu');
      ...
      validerListe(cr.liste, ou + '.liste', pb, (e, ou2) => {
        ...
        if (e.contactIdVendor !== undefined && (!estTexte(e.contactIdVendor) || (e.contactIdVendor !== '' && !RE_ID_VENDOR.test(e.contactIdVendor)))) {
          pb.push(ou2 + '.contactIdVendor : identifiant Vendor ou vide attendu');
        }
        if (e.idDeciplusVendor !== undefined && (!estTexte(e.idDeciplusVendor) || (e.idDeciplusVendor !== '' && !/^[0-9]{1,20}$/.test(e.idDeciplusVendor)))) {
          pb.push(ou2 + '.idDeciplusVendor : « ' + e.idDeciplusVendor + ' » n\'est pas un Id Deciplus');
        }
  ...
  M.LABELS.forEach((s) => {
    ...
    if (estObjet(b.clientsRetrouves)) {
      ...
      n.clientsRetrouves.liste = (b.clientsRetrouves.liste || []).map((e) => {
        ...
        // Contact Vendor : gardés seulement s'ils sont bien formés, sur toute ligne.
        if (RE_ID_VENDOR.test(String(e.contactIdVendor || ''))) o.contactIdVendor = String(e.contactIdVendor);
        if (/^[0-9]{1,20}$/.test(String(e.idDeciplusVendor || ''))) o.idDeciplusVendor = String(e.idDeciplusVendor);
        return o;
      });
    }
