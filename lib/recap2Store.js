...
const CLES_STUDIO = ['studio', 'nonReconduction', 'completion', 'clientsRetrouves', 'avertissements', 'controleBloquant'];
...
const CLES_CR = ['ventesSignees', 'annulesExclus', 'ventesValides', 'signataires', 'retrouves',
  'taux', 'tauxPct', 'liste', 'doublonsSignataire', 'retrouvesSansEncaissement'];
...
const CLES_CR_LIGNE = ['client', 'date', 'prestation', 'commercial', 'retrouve', 'site', 'dateVente', 'idClient', 'encaisse'];
  ...
  if (rapport.source !== undefined) {
    if (!estObjet(rapport.source)) pb.push('source : objet attendu');
      ...
      Object.keys(rapport.source).forEach((k) => {
        ...
        if (!/^deciplus_(ventes_)?\d{4}-\d{2}$/.test(k) && k !== 'fitnessBooster') pb.push('source : clé inattendue « ' + k + ' »');
...
function validerStudio(bloc, label, pb, version) {
  ...
  if (cr !== null && cr !== undefined) {
    ...
    if (!estObjet(cr)) pb.push(ou + ' : objet ou null attendu');
      ...
      ['ventesSignees', 'annulesExclus', 'ventesValides', 'tauxPct', 'doublonsSignataire', 'retrouvesSansEncaissement'].forEach((k) => {
      ...
      if (Array.isArray(cr.liste) && estNombre(cr.signataires) && cr.liste.length !== cr.signataires) {
        pb.push(ou + ' : ' + cr.liste.length + ' nom(s) pour ' + cr.signataires + ' signataire(s)');
      }
    }
  }
}
  ...
  M.LABELS.forEach((s) => {
    ...
    if (estObjet(b.clientsRetrouves)) {
      ...
      n.clientsRetrouves.liste = (b.clientsRetrouves.liste || []).map((e) => {
        const o = {};
        CLES_CR_LIGNE.forEach((k) => { o[k] = e[k] === undefined ? (k === 'retrouve' || k === 'encaisse' ? false : '') : e[k]; });
        // Ceinture et bretelles : un id ne survit au nettoyage que s'il est
        // plausible ET rattaché à une vente réellement retrouvée.
        if (!o.retrouve || !/^[0-9]{1,20}$/.test(String(o.idClient || ''))) o.idClient = '';
        return o;
      });
    }
    propre.studios[s] = n;
