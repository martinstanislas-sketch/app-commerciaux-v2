...
const SCHEMA = `
  ...
  -- Chaque version d'une remarque manuelle (création, modification, suppression).
  CREATE TABLE IF NOT EXISTS recap2_notes_historique (
    ...
    apres         TEXT NOT NULL DEFAULT '',   -- '' = supprimée
    le            TEXT NOT NULL,
...
function enregistrer(db, { mois, studio, type, client, idClient = '', idVendor = '', remarque, par = '', maintenant = new Date() } = {}) {
  ...
  const ancienne = db.prepare('SELECT remarque FROM recap2_notes WHERE mois = ? AND studio = ? AND type = ? AND cle_personne = ?').get(mois, s, type, cle);
  const avant = ancienne ? String(ancienne.remarque || '') : '';
  if (avant !== texte) {
    db.prepare(`INSERT INTO recap2_notes_historique (mois, studio, type, cle_personne, client, avant, apres, le, par) VALUES (?,?,?,?,?,?,?,?,?)`)
      .run(mois, s, type, cle, String(client || '').trim(), avant, texte, maintenant.toISOString(), String(par || '').slice(0, 80));
  }
  ...
  return versNote(db.prepare('SELECT * FROM recap2_notes WHERE mois = ? AND studio = ? AND type = ? AND cle_personne = ?')
    .get(mois, s, type, cle), metaHistorique(db, mois));
}

// `auteur` = qui l'a créée (première version de l'historique) ; `modifiee` = au
// moins une version a remplacé un texte existant. Les versions complètes restent
// en base (recap2_notes_historique), consultables par un administrateur.
function versNote(r, meta) {
  const m = (meta && r && meta.get(r.studio + '|' + r.type + '|' + r.cle_personne)) || {};
  const n = { remarque: (r && r.remarque) || '', modifieLe: (r && r.modifie_le) || '', modifiePar: (r && r.modifie_par) || '' };
  // Forme inchangée pour une remarque vide ; auteur et « modifiée » seulement si elle existe.
  if (n.remarque) Object.assign(n, { auteur: m.auteur || n.modifiePar, modifiee: !!m.modifiee });
  return n;
}
function metaHistorique(db, mois) {
  const meta = new Map();
  db.prepare('SELECT studio, type, cle_personne, avant, par FROM recap2_notes_historique WHERE mois = ? ORDER BY id').all(mois).forEach((h) => {
    const k = h.studio + '|' + h.type + '|' + h.cle_personne;
    const m = meta.get(k) || { auteur: '', modifiee: false };
    if (!m.auteur && h.avant === '') m.auteur = h.par;
    if (h.avant !== '') m.modifiee = true;
    meta.set(k, m);
  });
  return meta;
}
// La clé d'une personne, comme à l'écriture (Id Deciplus, contact Vendor, identité).
function clePersonne({ client, idClient = '', idVendor = '' }) {
  const id = idDe(idClient), vendor = idVendorDe(idVendor);
  return id ? 'id:' + id : vendor ? 'vendor:' + vendor : 'nom:' + cleIdentite(client);
}
// La remarque actuelle d'une personne (ou null) avec son auteur d'origine.
function noteActuelle(db, { mois, studio, type, client, idClient = '', idVendor = '' }) {
  const cle = clePersonne({ client, idClient, idVendor });
  const r = db.prepare('SELECT * FROM recap2_notes WHERE mois = ? AND studio = ? AND type = ? AND cle_personne = ?').get(mois, studio, type, cle);
  return r ? versNote(r, metaHistorique(db, mois)) : null;
}
...
function notesDuMois(db, mois) {
  ...
  return { parId, parNom, parVendor, meta: metaHistorique(db, mois) };
}
...
function appliquer(rapport, index) {
  ...
  Object.keys(copie.studios).forEach((s) => {
    TYPES.forEach((type) => {
      ...
      if (!liste) return;
      liste.forEach((l) => { l.note = versNote(noteDe(index, s, type, l), index.meta); });
    });
  });
  return copie;
}
...
function historique(db, mois) {
  return db.prepare('SELECT * FROM recap2_notes_historique WHERE mois = ? ORDER BY id').all(mois);
}

module.exports = { SCHEMA, TYPES, LONGUEUR_MAX, creerTable, enregistrer, notesDuMois, appliquer, ligneDe, historique, noteActuelle, clePersonne };
