...
// Les remarques déposées ne sont qu'un résumé : l'écran les RECALCULE depuis les
// champs structurés (public/recap2-regles.js). On n'accepte que des phrases du
// registre (ou des formulations des versions précédentes du moteur), jamais
// une note brute.
const REMARQUES_ANCIENNES = /^(Vérifie l’identité du client|Renseigne dans Deciplus la raison|Complète la suspension dans Deciplus|La suspension du client est terminée|Contacte le client|Réalise (un|un nouveau) bilan|Appelle le client pour comprendre|Échange avec le responsable du club|Vérifie la situation financière du client\. |Vérifie que l’écart de |Vérifie le montant du contrat : |Vérifie les encaissements du client : |Prolonge le contrat de \d+ jours dans Deciplus)/;
const remarqueAcceptee = (t) => Regles.texteDuRegistre(t) || REMARQUES_ANCIENNES.test(String(t));
...
function valider(depot, rapport, studiosConnus) {
  ...
  depot.resultats.forEach((r, i) => {
    ...
    if (!Array.isArray(a.remarques) || a.remarques.length > 6) pb.push(ou + '.analyse.remarques : 0 à 6 attendues');
    (a.remarques || []).forEach((t) => { if (!remarqueAcceptee(t)) pb.push(ou + ' : remarque hors gabarit'); });
    if (r.type === 'nr') {
...
function nettoyerAnalyse(a) {
  ...
  return {
    statut: String(a.statut || ''), contentieux: !!a.contentieux, irrecuperable: !!a.irrecuperable, recuperationOuverte: !!a.recuperationOuverte,
    retractation: !!a.retractation,
    ...
    finance: f ? { contratVendor: n(f.contratVendor), contratDeciplus: n(f.contratDeciplus), contratReference: n(f.contratReference), facture: n(f.facture),
      ...
      anomalie: court(f.anomalie, 30), justification: court(f.justification, 80), valide: false, vendorEstime: !!f.vendorEstime, limites: (f.limites || []).slice(0, 5).map((x) => court(x, 120)) } : null,
    reconduction: rc ? { produit: court(rc.produit, 80), debut: court(rc.debut, 10), mensuel: n(rc.mensuel) } : null,
    remarques: (a.remarques || []).slice(0, 6).map((x) => court(x, 400)),
  ...
  Object.keys(copie.studios).forEach((s) => {
    ...
    if (nr && Array.isArray(nr.liste)) {
      nr.liste.forEach((l) => {
        ...
        // Requalification (registre) : une rétractation ou un contentieux figés
        // « éligibles » par un contrôle d'une version précédente sont exclus.
        const q = Regles.requalifierNR(c.analyse);
        if (k && k.eligible === true && q && (q.contentieux || q.retractation)) {
          l.cohorte = Object.assign({}, k, { eligible: false, motifExclusion: q.motifExclusion, requalifie: true });
        }
