...
// VALIDATION EXPLICITE DE L'IDENTITÉ (arbitrage du 18/09/2026) : seule elle
// fait disparaître « Vérifie l'identité du prospect dans Vendor. » — une
// décision commerciale (Flex proposé, non intéressé…) ne la masque jamais.
// Historisée dans recap2_flex_historique (avant/après « identite:… »).
const SCHEMA_IDENTITE = `
  CREATE TABLE IF NOT EXISTS recap2_vni_identites (
    mois        TEXT NOT NULL,
    studio      TEXT NOT NULL,
    contact_id  TEXT NOT NULL,
    client      TEXT NOT NULL DEFAULT '',
...
`;
function creerTable(db) { db.exec(SCHEMA); db.exec(SCHEMA_MANUEL); db.exec(SCHEMA_IDENTITE); }

// valeur 'validee' (identité confirmée par un administrateur) ou 'auto' (retrait).
function validerIdentite(db, { mois, studio, contactId, valeur, client = '', par = '' }, maintenant = new Date().toISOString()) {
  ...
  if (valeur !== 'validee' && valeur !== 'auto') throw new Error('valeur : validee ou auto');
  const id = String(contactId);
  const qui = String(par).slice(0, 80);
  const deja = db.prepare('SELECT 1 FROM recap2_vni_identites WHERE mois = ? AND studio = ? AND contact_id = ?').get(mois, studio, id);
  const avant = deja ? 'identite:validee' : 'identite:a_verifier';
  const apres = valeur === 'validee' ? 'identite:validee' : 'identite:a_verifier';
  db.transaction(() => {
    if (valeur === 'validee') {
      db.prepare(`INSERT OR REPLACE INTO recap2_vni_identites (mois, studio, contact_id, client, modifie_le, modifie_par)
        VALUES (?, ?, ?, ?, ?, ?)`).run(mois, studio, id, String(client || ''), maintenant, qui);
    } else db.prepare('DELETE FROM recap2_vni_identites WHERE mois = ? AND studio = ? AND contact_id = ?').run(mois, studio, id);
    if (avant !== apres) {
      db.prepare(`INSERT INTO recap2_flex_historique (mois, studio, contact_id, client, avant, apres, le, par)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(mois, studio, id, String(client || ''), avant, apres, maintenant, qui);
    }
  })();
  return valeur === 'validee' ? { validee: true, modifieLe: maintenant, modifiePar: qui } : { validee: false };
}
function identitesDuMois(db, mois) {
  const idx = new Map();
  db.prepare('SELECT * FROM recap2_vni_identites WHERE mois = ?').all(mois).forEach((r) => {
    idx.set(r.studio + '|' + r.contact_id, { validee: true, modifieLe: r.modifie_le, modifiePar: r.modifie_par });
  });
  return idx;
}
...
function appliquer(rapport, controles, decisions = new Map(), identites = new Map()) {
  if (!rapport || !rapport.studios) return rapport;
  if ((!controles || !controles.size) && (!decisions || !decisions.size) && (!identites || !identites.size)) return rapport;
  ...
  Object.keys(copie.studios).forEach((s) => {
    ...
    v.liste.forEach((l) => {
      ...
      actifs.push(l);
      const idv = (identites && identites.get(cle)) || null;
      if (idv) l.identiteVni = idv;
      if (!c && !d) return;
      l.flex = {
...
module.exports = {
  ...
  creerTable, valider, enregistrer, controlesDuMois, decider, decisionsDuMois, appliquer, historique, validerIdentite, identitesDuMois,
};
