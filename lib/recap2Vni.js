...
//   · `transformesDepuis` : qui, et par quelle source (sans nom supplémentaire).
function appliquer(rapport, index, { connuesJusquau = '' } = {}) {
  ...
  Object.keys(copie.studios).forEach((s) => {
    ...
    v.liste.forEach((p) => {
      ...
      if (t) retires.push({ contactId: p.contactId || '', idClient: p.idClient || '', dateVenue: p.dateVenue, transformeLe: t.date, source: t.source });
      else actifs.push(p);
    });
