...
const ACTION_CAUSE = {
  prix: T.PRIX, temps: T.TEMPS, motivation: T.MOTIVATION, resultats: T.RESULTATS, planning: T.PLANNING,
  demenagement: T.DEMENAGEMENT, sante: T.SANTE, insatisfaction: T.INSATISFACTION, coach: T.COACH, impaye: T.IMPAYE,
  fin_challenge: T.FIN_CHALLENGE, frequentation: T.INCONNU, inconnu: T.INCONNU, autre: T.INCONNU,
};
...
function finance(ref, { vendor, journal, ph, suspension }) {
  ...
  if (f.contratVendor != null && f.contratDeciplus != null && Math.abs(f.contratVendor - f.contratDeciplus) >= SEUIL_ECART) {
    ...
    f.remarque = 'Vérifie le montant du contrat : Vendor prévoit ' + euros(f.contratVendor) + ', Deciplus ' + euros(f.contratDeciplus) + '.';
    return f;
  }
  if (f.encaisse != null && f.journalNet != null && Math.abs(f.encaisse - f.journalNet) >= SEUIL_ECART) {
    f.anomalie = 'encaisse_discordant';
    f.remarque = 'Vérifie les encaissements du client : Deciplus indique ' + euros(f.encaisse) + ' encaissés, le journal ' + euros(f.journalNet) + '.';
    return f;
  }
  ...
  else if (suspension && suspension.montantSuspendu >= f.ecart - SEUIL_ECART && suspension.raisonRenseignee) f.justification = 'la suspension autorisée';
  else { const j = JUSTIF_NOTE.find(([, re]) => ph.some((p) => re.test(p.norm))); if (j) f.justification = j[0]; }
  if (f.justification) {
    f.remarque = 'Vérifie que l’écart de ' + euros(f.ecart) + ' est bien justifié par ' + f.justification + '.';
  } else {
    f.remarque = 'Vérifie la situation financière du client. Le contrat signé prévoyait ' + euros(contrat) + ', mais '
      + euros(net) + ' ont réellement été réglés, soit un écart de ' + euros(f.ecart) + '.'
      + (f.facture != null && contrat - f.facture >= SEUIL_ECART ? ' La facturation Deciplus ne couvre que ' + euros(f.facture) + '.' : '');
  }
  return f;
}
...
function analyser({ ligne = {}, mois, aujourdHui, dossier, vendor = null, journal = null, mode = 'nr' }) {
  ...
  const fin = (statut, extra) => Object.assign(r, { statut }, extra || {});
  if (!/^[0-9]{1,20}$/.test(String(ligne.idClient || '')) || !dossier || !dossier.ok) {
    return fin('a_creuser', { indication: !ligne.idClient ? 'aucun identifiant Deciplus fiable' : 'dossier Deciplus illisible', remarques: [T.IDENTITE], motifExclusion: 'identité incertaine' });
  }
  ...
  if (ph.some((p) => DOUTE_IDENTITE.test(p.norm))) {
    return fin('a_creuser', { indication: 'une note évoque une autre fiche ou un doublon', remarques: [T.IDENTITE], motifExclusion: 'identité incertaine' });
  }
  ...
  if (susp) {
    r.suspension = susp;
    const pro = prolongation(cSusp);
    r.suspension.prolongation = pro;
    const remProlong = pro && pro.verifiable && pro.manqueJours > 0
      ? 'Prolonge le contrat de ' + pro.manqueJours + ' jours dans Deciplus pour compenser la suspension'
        + (susp.montantSuspendu > 0 ? ' (' + euros(susp.montantSuspendu) + ' d’échéances suspendues à replanifier).' : '.')
      : '';
    if (susp.terminee) {
      return fin('a_traiter', { indication: 'suspension terminée le ' + dateFr(susp.fin) + ' sans reprise',
        ...
        remarques: [T.SUSP_FINIE] });
    }
    if (!susp.raisonRenseignee) return fin('suspendu', { indication: 'suspension sans raison renseignée', remarques: [T.SUSP_RAISON].concat(remProlong ? [remProlong] : []), motifExclusion: 'suspension' });
    if (!susp.repriseDate && !susp.indeterminee) return fin('suspendu', { indication: 'suspension sans date de reprise', remarques: [T.SUSP_REPRISE].concat(remProlong ? [remProlong] : []), motifExclusion: 'suspension' });
    return fin('suspendu', { indication: 'suspension renseignée' + (susp.repriseDate ? ', reprise prévue le ' + dateFr(susp.repriseDate) : ', durée indéterminée'),
      remarques: remProlong ? [remProlong] : [], motifExclusion: 'suspension' });
  }
  ...
  if (raisonArret === 'RETRACTATION' && cause.code === 'inconnu') Object.assign(cause, { code: 'autre', certitude: 'Confirmée', indice: 'rétractation dans les 14 jours (Deciplus)' });
  r.cause = { code: cause.code, libelle: cause.code === 'autre' && raisonArret === 'RETRACTATION' ? 'Rétractation' : CAUSES[cause.code], certitude: cause.certitude, indice: cause.indice };

  const irr = ph.find((p) => IRRECUPERABLE.test(p.norm));
  r.finance = finance(ref, { vendor, journal, ph, suspension: null });
  if (irr) {
    return fin('resilie', { irrecuperable: true, indication: 'départ irrécupérable (note ' + ZONES[irr.zone] + (irr.date ? ' du ' + dateFr(irr.date) : '') + ')',
      remarques: [], motifExclusion: 'départ irrécupérable' });
  }
  const remarques = [];
  if (r.finance && r.finance.remarque) remarques.push(r.finance.remarque);
  if (cause.code === 'transfert') remarques.push(T.IDENTITE);
  else if (cause.certitude === 'À confirmer' && cause.code !== 'inconnu') remarques.push(T.A_CONFIRMER);
  else remarques.push(ACTION_CAUSE[cause.code] || T.INCONNU);
  ...
  // une décision manuelle « Résilié » existe (lue par public/recap2-conseils.js).
  r.recuperationOuverte = cause.code === 'demenagement' && cause.certitude !== 'À confirmer';
  return fin(cause.code === 'transfert' ? 'a_creuser' : 'a_traiter', { remarques: [...new Set(remarques)], motifExclusion: cause.code === 'transfert' ? 'identité incertaine' : '' });
}
