...
const NR = require('./recap2NrAnalyse.js');
...
function evaluerVente(vente, dossier, { aujourdHui } = {}) {
  const r = { prelevement: AV, reservation: AV, resilie: AV, alertes: [], regles: [], contrat: null, raison: '', details: {} };
  ...
  // CONTENTIEUX CONFIRMÉ (catégorie ou note, même règle que les non-reconduits) :
  // l'information part avec le contrôle ; le registre des remarques impose
  // alors le silence (aucune invitation à conserver le client).
  if (dossier.categorie != null || dossier.notes) {
    const ctx = NR.contentieux(dossier, NR.phrases(dossier.notes || {}, +String(aujourdHui).slice(0, 4)));
    if (ctx.confirme) Object.assign(r.details, { contentieux: true, contentieuxSource: ctx.source });
  }
  ...
  if (arrete && !remplacant) {
    ...
    else if (/UNPAID/.test(raisons)) {
      r.regles.push('R15'); r.alertes.push(ALERTES.INTERROMPU);
      const cr = rejets(ref).filter((e) => e.motifRejet);
      if (cr.length) r.details.codesRejet = cr.map((e) => ({ code: String(e.motifRejet).slice(0, 8), date: String(e.rejet || e.date || '').slice(0, 10) })).slice(-3);
    }
  ...
  if (rejets(courant).length) {
    r.alertes.push(ALERTES.REJET);
    // Code SEPA du dernier rejet (MD01, AM04…) : affiché à l'administrateur.
    r.details.codesRejet = rejets(courant).filter((e) => e.motifRejet).map((e) => ({ code: String(e.motifRejet).slice(0, 8), date: String(e.rejet || e.date || '').slice(0, 10) })).slice(-3);
  }
  if (fut.length) {
